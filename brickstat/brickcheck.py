#e.g. python brickcheck.py --name_for_run decals_ngc --rs rs0 --lepipv DR9.6.4
#SV_bricks.txt
...
def OneBrickClassify(brickname):
    ...
    if os.path.isfile(log_dir):
        if "MemoryError" in open(log_dir).read():
            f2 = open('./%s/stats/MemoryError-%s.txt'%(NAME_FOR_RUN,LEPIPV), 'a')
            f2.write(str(brickname)+'\n')
            f2.close()
            return 1
        else:
            f2 = open('./%s/stats/Processing-%s.txt'%(NAME_FOR_RUN,LEPIPV), 'a')
            f2.write(str(brickname)+'\n')
            f2.close()
            return 1
    else:
        f1 = open('./%s/stats/UnprocessedBricks-%s.txt'%(NAME_FOR_RUN,LEPIPV), 'a')
...
def BrickClassify():
    ...
    p = mp.Pool(N)
    global NAME_FOR_RUN
    f1 = open('./%s/stats/MemoryError-%s.txt'%(NAME_FOR_RUN,LEPIPV), 'w')
    f1.close()
    f2 = open('./%s/stats/Processing-%s.txt'%(NAME_FOR_RUN,LEPIPV), 'w')
    f2.close()
    bricks = np.loadtxt('./%s/UnfinishedBricks-%s.txt'%(NAME_FOR_RUN,LEPIPV), dtype=np.str)
